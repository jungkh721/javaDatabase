	...
	// ------------------- 위쪽은 사용자 인터페이스구현(UI) , 아래쪽은 method(동작) 처리 -----------------
		...
		btn4.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int ans=JOptionPane.showConfirmDialog(null, "작업을 계속하시겟습니까????", "작업종료창", JOptionPane.YES_NO_OPTION);
				if(ans ==0) {
					JOptionPane.showMessageDialog(null, "작업을 계속진행합니다");
				}
				else {
					JOptionPane.showMessageDialog(null, "작업을 계속진행합니다");
					System.exit(0);
		...
		btn5.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int ans=JOptionPane.showConfirmDialog(null, "작업을 계속하시겟습니까????", "작업종료창", JOptionPane.YES_NO_CANCEL_OPTION);
				if(ans ==0) {
					JOptionPane.showMessageDialog(null, "작업을 계속진행합니다");
				}
				else if(ans ==1) {
